...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="active-task-id">
...
describe("PomodoroContext", () => {
  it("provides default values", () => {
    ...
    expect(screen.getByTestId("is-running")).toHaveTextContent(
      "isRunning: false"
    );
  ...
  it("sets activeTaskId and increments completedPomodoros", async () => {
    ...
    expect(screen.getByTestId("completed-pomodoros")).toHaveTextContent(
  ...
  it("throws if usePomodoroContext is used outside of PomodoroProvider", () => {
    ...
    expect(() => render(<FaultyComponent />)).toThrow(
      /must be used within PomodoroProvider/i
    );
  });
